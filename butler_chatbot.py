...
def enhanced_extract_text_from_pdf(pdf_path, is_url=False):
    ...
    try:
        ...
        for page_num in range(len(pdf_reader.pages)):
            ...
            text_by_page.append({"page": page_num + 1, "text": page_text})
...
"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは文書分析と要約の専門家です。包括的で正確かつ構造化された要約を日本語で作成します。",
                },
...
def identify_pdf_topics(pdf_chunks, client):
    ...
    try:
        ...
        combined_text = "\n\n".join(
            [chunk for chunk in pdf_chunks[: min(5, len(pdf_chunks))]]
        )
...
"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたはコンテンツ分析とトピック特定の専門家です。日本語で回答してください。",
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=300,
...
"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは正確で詳細な調査アシスタントです。文書内容に厳密に基づいた詳細な回答を日本語で提供します。",
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=350,
...
"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは議論、修辞、談話の分析の専門家です。日本語で回答してください。",
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=400,
...
def get_gpt_general_response(input_text):
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "あなたは上品で丁寧な執事です。常に日本語で丁寧な口調で話し、「お嬢様」と呼びかけます。役立つ情報を提供し、知識豊かでサポート的であるように心がけてください。あなたの応答は執事の献身と敬意を反映するものでなければなりません。決して嘘をつかず、正確な情報のみを伝えてください。",
            }
        ]
...
def get_gpt_emotional_response(input_text):
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "あなたは上品で丁寧な執事です。お嬢様の感情を察して適切に応答してください。常に日本語で丁寧に、優しく応答し、決して嘘をつかないでください。",
            },
            {
                "role": "user",
                "content": f"お嬢様が「{input_text}」と言っています。感情に配慮した返答をお願いします。",
            },
        ]
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは上品で丁寧な執事です。常にお嬢様に敬意を持って話します。日本語で応答してください。",
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=200,
        ...
        print(f"API error: {e}")
        return (
            "お嬢様、申し訳ございません。ただいま情報を処理できない状況でございます。"
        )
...
def get_butler_response(input_text):
    ...
    if api_available:
        ...
        if ref_response:
            ...
            if (
                len(input_text) > 5 and len(ref_response) > 20
            ):  # Only cache meaningful exchanges
...
def chat_with_butler():
    ...
    while True:
        ...
        if (
            len(user_input) > 10
            or "pdf" in user_input.lower()
            or "url" in user_input.lower()
        ):
